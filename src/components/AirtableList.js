...
function AirtableList() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const sortedRecords = response.data.records
          .map(record => {
            // fields nesnesinin varlığını kontrol et
            if (!record.fields) {
          ...
          .filter(record => record !== null) // Eksik kayıtları filtrele
  ...
  const sendSMS = async (phone, customMessage) => {
    try {
      ...
      if (response.data.success) {
        ...
        try {
          const statusResponse = await axios.get(`http://localhost:5000/api/sms-status/${jobid}`);
  ...
  const sendBulkEmails = async (customMessage) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/send-bulk-emails', recipients.map(recipient => ({
        ...recipient,
        message: customMessage
      })));
  ...
  const sendBulkSMS = async (customMessage) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/send-bulk-sms', recipients);
  ...
  if (loading) return <div className="text-center text-gray-600 text-lg">Yükleniyor...</div>;
    ...
    <div className="container mx-auto p-6">
      <div className="bg-white shadow-2xl rounded-xl p-8">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-3xl font-bold text-gray-800">Katılımcı Listesi</h2>
          <div className="flex space-x-3">
            <button
              onClick={() => exportToCSV(records)}
              className="px-5 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg shadow-md hover:from-green-600 hover:to-green-700 transition-all duration-200 text-sm font-semibold"
            >
              CSV İndir
            </button>
            <button
              onClick={sendBulkMessages}
              className="px-5 py-2 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-lg shadow-md hover:from-purple-600 hover:to-purple-700 transition-all duration-200 text-sm font-semibold disabled:bg-gray-400 disabled:cursor-not-allowed"
            ...
            <button
              ...
              className="px-5 py-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg shadow-md hover:from-orange-600 hover:to-orange-700 transition-all duration-200 text-sm font-semibold disabled:bg-gray-400 disabled:cursor-not-allowed"
            ...
            <button
              ...
              className="px-5 py-2 bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-lg shadow-md hover:from-teal-600 hover:to-teal-700 transition-all duration-200 text-sm font-semibold disabled:bg-gray-400 disabled:cursor-not-allowed"
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Ad Soyad</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Son Katılım</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {records.map((record, index) => (
                <tr key={record.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{record.fields['AD SOYAD'] || '-'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {record.fields['TELEFON'] || '-'}
                    {!isValidPhoneNumber(record.fields['TELEFON']) && (
                      <span className="text-red-600 text-xs ml-2">(Geçersiz)</span>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {record.fields['E POSTA'] || '-'}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap flex space-x-3">
                    <button
                      ...
                      className="px-4 py-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg shadow-sm hover:from-blue-600 hover:to-blue-700 transition-all duration-200 text-sm font-medium disabled:bg-gray-400 disabled:cursor-not-allowed"
                    ...
                    <button
                      ...
                      className="px-4 py-1 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg shadow-sm hover:from-orange-600 hover:to-orange-700 transition-all duration-200 text-sm font-medium disabled:bg-gray-400 disabled:cursor-not-allowed"
                    ...
                    <button
                      ...
                      className="px-4 py-1 bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-lg shadow-sm hover:from-teal-600 hover:to-teal-700 transition-all duration-200 text-sm font-medium disabled:bg-gray-400 disabled:cursor-not-allowed"
                      disabled={!isValidPhoneNumber(record.fields['TELEFON'])}
                    >
                      SMS Gönder
                    </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 w-full max-w-lg shadow-2xl">
            ...
            <textarea
              className="w-full p-4 border border-gray-300 rounded-lg mb-6 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 resize-none"
              rows="5"
            ...
            <div className="flex justify-end space-x-3">
              <button
                onClick={closeModal}
                className="px-5 py-2 bg-gray-200 text-gray-700 rounded-lg shadow-sm hover:bg-gray-300 transition-all duration-200 font-medium"
              >
                İptal
              </button>
              <button
                onClick={handleModalSend}
                className="px-5 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg shadow-md hover:from-blue-600 hover:to-blue-700 transition-all duration-200 font-medium"
              >
                Gönder
              </button>
            </div>
